...
- 合辑按原视频时间向前剪辑：同一时刻相近角度的提名合为一个分镜；全景取景方向明显分离时，允许同一时间段以不同方向分别剪入，即使是同一个人在移动。相邻时间组只缩减补足的前后文，避免无意义的重播。
...
评委请求不会发送 `max_tokens` 或 `max_completion_tokens`，输出上限由模型服务商决定。若界面显示评委失败，请查看本次目录的 `jury/candidate*.json`：`invalid_review` 表示回复不符合 JSON/时间协议，`error.detail` 会指出 `facts` 的数组形状、字段、时间或描述问题；`timeout` 表示单次读取超过 120 秒，按设置直接弃权。格式错误最多针对具体问题补试一次，每位评委仍最多调用两次。审计文件只记录安全错误类别和用量，不保存原始失败回复。
